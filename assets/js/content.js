...
$(function () {
	...
	waitForElm(doc, '#microConsole-Logs').then(async (elm) => {
		...
		iframeDoc.head.appendChild(linkjsoneditor);
		iframeDoc.querySelector("#query-detail-page").appendChild(modal);
		...
		function cargarJsonView() {
			iframeDoc.querySelectorAll("div[class='logs-insights-expanded-row']>table>tbody>tr").forEach(r => {
				var tdTitle = r.children[0].innerText;
				var tdValue = r.children[1].innerText;
				if (tdTitle.includes("@message")) {
					try {
						var json
						if (tdValue.includes("INFO EVENT: ") && tdValue.includes("{") && tdValue.includes("}") && tdValue.includes("\"")) {
...
convert to json
{  user: 'postgres',  password: 'YQQ2e4NLdUNs8WFb',  host: 'dev-wekall-cc-pg11.cluster-ro-cvdtlcbmntuu.us-east-1.rds.amazonaws.com',  database: 'customers',  port: '5432'}

//funcion para covertir string sin comillas {  user: 'postgres',  password: 'YQQ2e4NLdUNs8WFb',  host: 'dev-wekall-cc-pg11.cluster-ro-cvdtlcbmntuu.us-east-1.rds.amazonaws.com',  database: 'customers',  port: '5432'} a json
